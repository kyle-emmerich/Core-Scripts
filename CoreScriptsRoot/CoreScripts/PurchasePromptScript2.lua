...
local VR_BUTTON = 'rbxasset://textures/ui/VR/buttonMockup.png'
local VR_BUTTON_DOWN = 'rbxasset://textures/ui/VR/buttonSelectedMockup.png'
...
local function create3DGui()
	...
	Gui.Dialog:SetContent(Gui.PurchaseDialog)
		...
		Gui.ContainerFrame.Parent = Gui.PurchaseDialog
			...
			Gui.OkPurchasedButton = createImageButtonWithText3D("OkPurchasedButton", UDim2.new(0, 0.25, 0, 225), VR_BUTTON, VR_BUTTON_DOWN, "Ok", Enum.Font.SourceSansBold, Enum.FontSize.Size36)
...
local function setPurchaseDataInGui(isFree, invalidBC)
	...
	if not isFree then
		if PurchaseData.CurrencyType == Enum.CurrencyType.Tix and not IsVRMode then
...
local function switchGui(activeGui)
	...
	if activeGui then
		...
		if IsCurrentlyPrompting then
			onPurchasePrompt(assetId, productId, currencyType, equipOnPurchase)
		end
...
local parts = workspace:WaitForChild("PromptParts")
for i, v in pairs(parts:GetChildren()) do
	if v.Name:sub(1, 1) == "a" then
		v.Touched:connect(function(hit)
			if hit.Parent == game.Players.LocalPlayer.Character then
				onPurchasePrompt(game.Players.LocalPlayer, tonumber(v.Name:sub(2)), false, Enum.CurrencyType.Robux, nil)
			end
		end)
	elseif v.Name:sub(1, 1) == "p" then
		v.Touched:connect(function(hit)
			if hit.Parent == game.Players.LocalPlayer.Character then
				onPurchasePrompt(game.Players.LocalPlayer, nil, false, Enum.CurrencyType.Robux, tonumber(v.Name:sub(2)))
			end
		end)
